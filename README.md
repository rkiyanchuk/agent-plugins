...
| Plugin | Description |
|--------|-------------|
| [`init-project`](./init-project) | Bootstrap Claude Code in a project (a `/init` superset) — write `CLAUDE.md`, build a codegraph index for large source trees, and interactively enable the right plugins & MCP servers with defaults suggested from the repo. Adds `/init-project`. |
